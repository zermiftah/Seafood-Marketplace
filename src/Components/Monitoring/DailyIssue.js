...
export default function DailyIssue() {
  ...
  const handleBackClick = () => {
    ...
    window.location.href = `http://172.16.206.4:3002/token?token=${token}`;
  };
  ...
  const handleLogout = async () => {
    ...
    try {
      const response = await axios.put(
        "http://172.16.206.4:2001/updateFI_item",
        data
      );
  ...
  return (
    <div className="flex flex-col items-center justify-center h-screen bg-gray-50 text-gray-800">
      <div className="max-w-full mx-auto py-10 px-4 sm:px-6 lg:px-8 w-full overflow-auto">
